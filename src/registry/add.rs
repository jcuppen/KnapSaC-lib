...
impl Registry {
    ...
    pub fn add_dependency_to_module(&mut self, identifier: &str, dependency_identifier: &str) {
    ...
    pub fn add_dependency_to_executable(&mut self, source_path: &Path, dependency_identifier: &str) {
        if let Some(e) = self.executables.get_mut(source_path) {
            e.add_dependency(dependency_identifier);
        }
        self.save();
    }
}
